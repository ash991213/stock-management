...
describe('FairLockService Test', () => {
    let module: TestingModule;
    let service: FairLockService;
    ...
    beforeAll(async () => {
        module = await Test.createTestingModule({
            ...
            providers: [FairLockService, StockRepository],
        }).compile();

        service = module.get<FairLockService>(FairLockService);
    ...
    describe('updateStock', () => {
        it('정상적인 재고 감소 처리', async () => {
            ...
            const queueKey1 = cacheService.getQueueKey(1);
            const queueKey2 = cacheService.getQueueKey(2);
            const lockKey1 = cacheService.getLockKey(1);
            const lockKey2 = cacheService.getLockKey(2);

            expect(await cacheService.llen(queueKey1)).toBe(0);
            expect(await cacheService.llen(queueKey2)).toBe(0);
            expect(await cacheService.get(lockKey1)).toBeNull();
            expect(await cacheService.get(lockKey2)).toBeNull();
        });
        ...
        it('재고 부족 시 예외 발생 및 롤백', async () => {
            ...
            // 락과 큐가 정리되었는지 확인
            const queueKey = cacheService.getQueueKey(3);
            const lockKey = cacheService.getLockKey(3);
            expect(await cacheService.llen(queueKey)).toBe(0);
            expect(await cacheService.get(lockKey)).toBeNull();
        });
        ...
        it('DB 업데이트 실패 시 캐시 롤백', async () => {
            ...
            const queueKey = cacheService.getQueueKey(1);
            const lockKey = cacheService.getLockKey(1);
            expect(await cacheService.llen(queueKey)).toBe(0);
            expect(await cacheService.get(lockKey)).toBeNull();
        });
